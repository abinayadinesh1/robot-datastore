...
/// Run the H.264 TCP capture with exponential backoff reconnection.
pub async fn run_h264_capture(
    h264_addr: &str,
    ...
    loop {
        ...
        match consume_h264_stream(h264_addr, &tx, robot_id).await {
            Ok(()) => {
...
async fn consume_h264_stream(
    ...
    tx: &mpsc::Sender<TimestampedFrame>,
    robot_id: &str,
) -> Result<(), H264CaptureError> {
    let mut stream = TcpStream::connect(addr)
        .await
        .map_err(|e| H264CaptureError::TcpConnect(e.to_string()))?;
    ...
    loop {
        let n = stream
            .read(&mut read_buf)
            .await
            .map_err(|e| H264CaptureError::TcpStream(e.to_string()))?;
        if n == 0 {
        ...
        while ts_buf.len() >= TS_PACKET_SIZE {
            ...
            if let Some(access_unit) = pes_assembler.push_ts_packet(&packet) {
                ...
                debug!(seq, nal_type, robot_id, "captured H.264 frame");
...
pub fn detect_nal_type(data: &[u8]) -> u8 {
    let mut i = 0;
